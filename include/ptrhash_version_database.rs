...
impl CsrAccessions {
    ...
    pub fn build(num_elements: usize, accessions_by_idx: &HashMap<usize, Vec<u32>>) -> Self {
        ...
        for i in 0..num_elements {
            ...
            if let Some(accs) = accessions_by_idx.get(&i) {
                for &acc in accs {
                    data.push(acc);
                }
    ...
    pub fn load(path: &str) -> Result<Self> {
        let data = std::fs::read(path).context("Cannot read CSR accession file")?;

        let num_elements = u64::from_le_bytes(data[0..8].try_into().unwrap()) as usize;
        let data_len = u64::from_le_bytes(data[8..16].try_into().unwrap()) as usize;

        let offsets_bytes = &data[16..16 + (num_elements + 1) * 4];
        ...
        let data_bytes = &data[data_start..data_start + data_len * 4];
...
/// The PtrHash MPHF wrapped in a MemCase for mmap-backed zero-copy deserialization.
/// We store it as an enum so we can hold either an owned PtrHash (during build)
/// or an mmap-backed one (during load).
enum MphfHolder {
    Owned(PtrHash),
    Mmap(MemCase<epserde::deser::DeserType<'static, PtrHash>>),
}

impl MphfHolder {
    fn index(&self, key: &u64) -> usize {
        match self {
            MphfHolder::Owned(ph) => ph.index(key),
            MphfHolder::Mmap(mc) => mc.index(key),
        }
    }
}
...
impl KmerDatabase {
    ...
    pub fn save(&self, prefix: &str) -> Result<()> {
        ...
        // MPHF — save using epserde for fast mmap loading
        {
            let mphf_path = format!("{}.mphf", prefix);
            match &self.mphf {
                MphfHolder::Owned(ph) => {
                    let mut f = File::create(&mphf_path)?;
                    ph.serialize(&mut f)
                        .map_err(|e| anyhow::anyhow!("Failed to serialize PtrHash: {}", e))?;
                }
                MphfHolder::Mmap(_) => {
                    anyhow::bail!("Cannot re-save an mmap-loaded database");
                }
            }
        }
    ...
    /// Load from disk — uses mmap for the MPHF (near-instant).
    pub fn load(prefix: &str) -> Result<Self> {
        ...
        eprintln!("  meta: {:.3}s", load_start.elapsed().as_secs_f64());

        // MPHF — load via epserde mmap (near-instant)
        let t = std::time::Instant::now();
        let mphf = {
            let mphf_path = format!("{}.mphf", prefix);
            let mc: MemCase<_> = <PtrHash>::mmap(&mphf_path, Flags::empty())
                    .map_err(|e| anyhow::anyhow!("Cannot mmap .mphf file: {}", e))?;
            MphfHolder::Mmap(mc)
        };
        eprintln!("  mphf: {:.3}s", t.elapsed().as_secs_f64());

        // Fingerprints — bulk read
        let t = std::time::Instant::now();
        let fingerprints = {
        ...
        eprintln!("  fingerprints: {:.3}s", t.elapsed().as_secs_f64());
        ...
        eprintln!("  taxid_indices: {:.3}s", t.elapsed().as_secs_f64());

        // TaxID mapping — bulk read
        let t = std::time::Instant::now();
        let index_to_taxid = {
        ...
        eprintln!("  taxmap: {:.3}s", t.elapsed().as_secs_f64());
        ...
        eprintln!("  accessions: {:.3}s", t.elapsed().as_secs_f64());
        eprintln!("  total load: {:.3}s", load_start.elapsed().as_secs_f64());
...
impl KmerDatabaseBuilder {
    ...
    pub fn build(&self, kmers: &[ExtractedKmer], _num_threads: usize) -> Result<KmerDatabase> {
        ...
        let mut accessions_by_idx: HashMap<usize, Vec<u32>> = HashMap::new();

        for (&kmer, info) in &minimizer_map {
            let idx = mphf.index(&kmer);
            if idx < num_minimizers {
                fingerprints[idx] = compute_fingerprint(kmer);
                taxid_indices[idx] = info.taxid_idx;
